...
def _group_texts(examples, block_size): #, bos, eos):
  ...
  # concatenated_examples = examples['input_ids']  # if batch=False   (batch=True로 하면 안될때가 있는데, 이거로 바꾸면 됨. //아니다 결과가 이상함)
  concatenated_examples = list(itertools.chain(* examples['input_ids']))  # if batch=True     
  num0 =(len(concatenated_examples)// block_size)
  concatenated_examples = concatenated_examples[:num0 * block_size]
  ...
  # print(len(_values))
  if len(set(len(x) for x in _values)) !=1:
    print(_values)
    for x in _values:
      print(len(x))
    raise Exception()
  result['input_ids'] = _values#[:10]
  result['attention_mask'] = _attn_masks#[:10]
  return result


def get_dataset(
    dataset_name, tokenizer, wrap, mode, cache_dir,
    block_size=512, num_proc=len(os.sched_getaffinity(0)), streaming=False):
  if wrap:
  ...
  if dataset_name == 'dolma_flatten-train':
      dataset = datasets.load_dataset(
        'json',
        # data_files="dataset/dolma_flatten/v1_5r2_sample-0000.json.gz",
        data_files="dataset/dolma_flatten/*.json.gz",
        cache_dir= cache_dir,
        split='train[:-100000]',
      )
  elif dataset_name == 'dolma_flatten-valid':
      dataset = datasets.load_dataset(
        'json',
        # data_files="dataset/dolma_flatten/v1_5r2_sample-0000.json.gz",
        data_files="dataset/dolma_flatten/*.json.gz",
  ...
  elif dataset_name == 'wiki_ch-train':
      dataset = datasets.load_from_disk('dataset/wiki_ch')['train']
      ind= torch.arange(len(dataset))[:-100000]
      dataset = dataset.select(ind)
  elif dataset_name == 'wiki_ch-valid':
      dataset = datasets.load_from_disk('dataset/wiki_ch')['train']
      ind= torch.arange(len(dataset))[-100000:]
      dataset = dataset.select(ind)
  ...
  if dataset_name in ['dolma_flatten-train', 'dolma_flatten-valid', 'wiki_ch-train','wiki_ch-valid','pile_github-train','pile_github-valid']:
    data = dataset
  else:
    data = dataset[mode]
  ...
  if 'dolma_flatten' in dataset_name:
    tokenized_dataset = tokenized_dataset.remove_columns(
      ['text', 'id','source'])
  elif 'wiki_ch' in dataset_name:
    tokenized_dataset = tokenized_dataset.remove_columns(
      ['text', 'id', 'title', 'url', 'revid'])
  ...
  tokenized_dataset = tokenized_dataset.filter(lambda example: len(example['input_ids']) >= block_size)
...
def get_dataloaders(config, tokenizer, skip_train=False,
  ...
  if skip_valid:
    ...
    if config.valid_num_limit>0:
      valid_set = valid_set.select(range(config.valid_num_limit))
